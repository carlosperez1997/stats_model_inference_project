...
if (log_price) {
  y_train <- log10(train$price)
} else{
  y_train <- train$price/1000
}

x_test <- select(test, -price)
y_test_orig <- test$price
if (log_price) {
  y_test <- log10(test$price)
} else{
...
```{r}
regression_metrics <- function(model, data, y_real, y_wout_log=NULL) {
...
# TEST
q <- regression_metrics(fit.lasso, x_test_df, y_test, y_test_orig)

# TRAIN
q <- regression_metrics(fit.lasso, x_train_df, y_train, y_train_orig)
```
